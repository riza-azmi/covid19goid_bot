...
def start(update, context):
    ...
    update.message.reply_text(
        ...
        "Semoga kamu sehat-sehat selalu. ", reply_markup=markup,parse_mode=telegram.ParseMode.MARKDOWN)
        
    mycursor = mydb.cursor()

    try:
        sql = "INSERT INTO covid19goid_bot (chat_id, pilihan, galat) VALUES (%s, %s, %s)"
        val = (str(update.message.chat_id), "start", "NULL")
...
def regular_choice(update, context):
    ...
    if x == 'A':
        try:
            ...
            mes = "Data Sebaran (" + str((datetime.now()+ timedelta(hours=7)).strftime("%d-%m-%Y %H:%M:%S")) + ")\n\n"
            i = 1
            
            for s in soup.find('div', class_="fusion-layout-column fusion_builder_column fusion_builder_column_1_4 fusion-builder-column-4 fusion-one-fourth fusion-column-first 1_4").find_all('p'):
                if s.text.strip() != '':
                    if i == 1:
                        mes += '*' + s.text.strip().strip(' ') + '*\n\n'
                    else:
                        mes += "".join(s.text.strip(' ').strip().split(" ")[0:-1]) + ' *' + "".join(s.text.strip(' ').strip().split(" ")[-1]) + '*\n'
                i += 1 

            i = 1
            mes += '\n\n'
            for s in soup.find('div', class_="fusion-layout-column fusion_builder_column fusion_builder_column_1_4 fusion-builder-column-5 fusion-one-fourth 1_4").find_all('p'):
                if s.text.strip(' ' ) != '':
                    if i == 1:
                        mes += '*' + s.text.strip().strip(' ') + '*\n\n'
                    else:
    ...
    try:
        sql = "INSERT INTO covid19goid_bot (chat_id, pilihan, galat) VALUES (%s, %s, %s)"
        val = (str(update.message.chat_id), "regular_choice: " + x, "NULL")
        mycursor.execute(sql, val)
        mydb.commit()
    except Exception as e:
        print("Error DB: " + str(e))
...
def received_information(update, context):
    ...
    try:
        sql = "INSERT INTO covid19goid_bot (chat_id, pilihan, galat) VALUES (%s, %s, %s)"
        val = (str(update.message.chat_id), "received_information: " + x, "NULL")
        mycursor.execute(sql, val)
        mydb.commit()
    except Exception as e:
        print("Error DB: " + str(e) + str(e))
...
def main():
    ...
    def error(update, context):
        ...
        update.message.reply_text("Ketik /start untuk memulai kembali.")
        logger.warning('Update "%s" caused error "%s"', update, context.error)
        update.message.bot.send_message(chat_id=406089265, text='Ada error') 
        mycursor = mydb.cursor()
        try:
            sql = "INSERT INTO covid19goid_bot (chat_id, pilihan, galat) VALUES (%s, %s, %s)"
            val = (str(update.message.chat_id), "error", 'Updater: ' + str(update) + '\nError:' + str(context.error))
            mycursor.execute(sql, val)
            mydb.commit()
        except Exception as e:
            print("Error DB: " + str(e))
